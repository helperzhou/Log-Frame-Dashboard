...
if st.session_state.role == "admin":
    ...
    # ================== INTERVENTION TRACKER SECTION ==================
        ...
        areas_of_support = list(enterprise_data)
        ...
        interventions = enterprise_data[selected_area_of_support]
...
# ================== NON-ADMIN VIEW ==================
    ...
    if "selected_province" not in st.session_state:
        st.session_state.selected_province = hub_province_map[st.session_state.selected_hub]
    if "selected_area_of_support" not in st.session_state:
        st.session_state.selected_area_of_support = list(enterprise_data[st.session_state.selected_enterprise].keys())[
            0]
    if "selected_intervention" not in st.session_state:
        st.session_state.selected_intervention = \
        enterprise_data[st.session_state.selected_enterprise][st.session_state.selected_area_of_support][0]
    ...
    if st.session_state.role == "admin":
            ...
            if structured_df is not None:
                ...
                pie_chart_data = {
                    ...
                    "series": [{
                        "name": "Total Targets",
                        "colorByPoint": True,
                        "data": [{"name": row["Output"], "y": row["Total Targets"]} for _, row in
                                 section_totals.iterrows()]
                    }]
                }
                hct.streamlit_highcharts(pie_chart_data)
            ...
            if structured_df is not None:
                ...
                for hub in structured_df.columns[3::9]:  # Extract hub names dynamically
                    if hub in structured_df.columns:
                        ...
                        if selected_quarter == "Q1":
                            selected_columns.extend(
                                [col for col in [f"{hub} Q1 Target", f"{hub} Q1 Actual"] if
                                 col in structured_df.columns])
                        elif selected_quarter == "Q2":
                            selected_columns.extend([col for col in [f"{hub} Q1 Target", f"{hub} Q1 Actual",
                                                                     f"{hub} Q2 Target", f"{hub} Q2 Actual"] if
        ...
        # ================== INTERVENTION TRACKER SECTION ==================
            ...
            if "selected_province" not in st.session_state:
                st.session_state.selected_province = hub_province_map[st.session_state.selected_hub]
            if "selected_area_of_support" not in st.session_state:
                st.session_state.selected_area_of_support = \
                list(enterprise_data[st.session_state.selected_enterprise].keys())[0]
            if "selected_intervention" not in st.session_state:
                st.session_state.selected_intervention = \
                enterprise_data[st.session_state.selected_enterprise][st.session_state.selected_area_of_support][0]
    ...
    # ================== NON-ADMIN VIEW ==================
        ...
        if "selected_area_of_support" not in st.session_state:
            st.session_state.selected_area_of_support = \
            list(enterprise_data[st.session_state.selected_enterprise].keys())[
                0]
        if "selected_intervention" not in st.session_state:
            st.session_state.selected_intervention = \
                enterprise_data[st.session_state.selected_enterprise][st.session_state.selected_area_of_support][0]
        ...
        # Logout Button
        st.button("🚪 Logout", on_click=logout, key="logout_user")
